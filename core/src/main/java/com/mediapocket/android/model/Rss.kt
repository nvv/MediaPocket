    ...
    init {
        ...
        channel.children.filter { node -> node.name == "item" }.forEach {
            item -> items.add(Item(title, item.getChild("title").value,
                ...
                if (item.hasChild("enclosure") && item.getChild("enclosure").getAttribute("length") != null && item.getChild("enclosure").getAttribute("length") != "None") item.getChild("enclosure").getAttribute("length").trim().toLong() else -1,
